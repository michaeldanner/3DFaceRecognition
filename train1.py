...
def parse_function(example_proto):
    ...
    #img = tf.reshape(img, shape=(350, 350, 3))
...
dataset = tf.data.TFRecordDataset('femaleface_train.tfrecord')
...
for epoch in range(EPOCHS):
    ...
    while True:
        ...
        if img.shape[0] != batch_size * batch_multiplier or img.shape[0] != label.shape[0]:
            ...
            break
        step += 1
        accuracy, train_loss, inference_loss, regularization_loss = train_step(img, label, reg_coef)
        if step % 10 == 0:
